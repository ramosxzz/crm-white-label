-- Metricas do funil: por etapa, quantidade de negocios, valor e tempo medio.
-- Sem security definer: depende da RLS de leads/pipeline_stages (igual
-- dashboard_stage_counts), entao cada usuario so ve o proprio tenant.
create or replace function public.funnel_metrics(p_tenant_id uuid, p_pipeline_id uuid default null)
returns table (
  stage_id uuid,
  stage_name text,
  stage_color text,
...
grant execute on function public.funnel_metrics(uuid, uuid) to authenticated, service_role;
