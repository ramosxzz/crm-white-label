  ...
  where c.tenant_id = p_tenant_id
    ...
    and (
      ...
      or (
        ...
        and regexp_replace(coalesce(l.phone, ''), '[^0-9]', '', 'g') like '%' || params.search_digits || '%'
      )
    )
...
grant execute on function public.list_chat_conversations(uuid, integer, text, text) to authenticated, service_role;
